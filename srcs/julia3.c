...
/*   Updated: 2018/10/23 00:49:36 by baudiber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

void	juliargh(t_xy *xy, t_setup *stp, int tid)
{
	double	rsqr;
	double	isqr;
	int		i;
	...
	while (rsqr + isqr < 4 && i < stp->tmp[tid].max_iter)
	{
		...
		stp->tmp[tid].z_i = 4 * stp->tmp[tid].z_i * stp->tmp[tid].tmp \
			+ stp->tmp[tid].c_i;
		rsqr = stp->tmp[tid].z_r * stp->tmp[tid].z_r;
		isqr = stp->tmp[tid].z_i * stp->tmp[tid].z_i;
		i++;
